...
*/
        ...
        case STACKSEG:
        {
            data = reinterpret_cast<BYTE *>(&stack[(_ss_VMA - address + 3)/4 -1])[(4 - ((_ss_VMA - address) % 4)) % 4];
            return data;
...
void MMU::set_byte(int address, BYTE data)
{
    ...
    switch (VMA2Seg(address))
    {
        ...
        case STACKSEG:
        {
            ...
            reinterpret_cast<BYTE *>(&stack[(_ss_VMA - address + 3)/4 - 1])[(4 - ((_ss_VMA - address) % 4)) % 4] = data;
            break;
        }
    	default:
    		break;
